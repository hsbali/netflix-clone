...
const Navbar = () => {
  ...
  return (
    <header>
      <div
        className={`${styles["main-nav"]} d-flex flex-column flex-md-row gap-4 align-items-center p-3 w-100`}
      >
              ...
              return (
                <Fragment key={i}>
                  <li onClick={() => setActiveCategoryNav(item)}>
                    <Link
                      to={item.link}
                      className={activeCategoryNav && activeCategoryNav.link === item.link ? `${styles.active}` : ""}
                    >
                      {item.text}
                    </Link>
                  </li>
